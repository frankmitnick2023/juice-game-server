...
async function initDB() {
  ...
  try {
    ...
    // Seed Games (Real Data + Categories)
    ...
    // Seed Courses (Real Timetable - Only if empty)
    ...
    if (parseInt(courseRows[0].count) === 0) {
        ...
        const courses = [
          ...
          {name: 'RAD Primary', 'Saturday', '09:30', '11:00', 'Carrie', 240, 25, 'Classroom 1', '5'},
          {name: 'RAD Grade 1', 'Saturday', '09:30', '10:30', 'Carrie', 230, 25, 'Classroom 2', '7'},
          {name: 'Open Acro', 'Saturday', '10:00', '11:00', 'Forrest', 230, 25, 'Classroom 3', '9+'},
          {name: 'PBT Technique', 'Saturday', '10:30', '11:30', 'Carrie', 230, 25, 'Classroom 2', '5-7'},
          {name: 'RAD Beginner', 'Saturday', '11:00', '12:00', 'Demi', 230, 25, 'Classroom 1', '3-4.5'},
          {name: 'K-Pop Teens', 'Saturday', '11:00', '12:30', 'Hazel', 240, 25, 'Classroom 4', '11-16'},
          {name: 'Open Ballet', 'Saturday', '11:00', '12:00', 'Tonia', 230, 25, 'Classroom 3', '9+'},
          {name: 'NZAMD Jazz L1', 'Saturday', '12:00', '13:00', 'Katie', 230, 25, 'Classroom 1', '5-6'},
          {name: 'RAD Grade 2', 'Saturday', '12:00', '13:00', 'Demi', 230, 25, 'Classroom 2', '8'},
          {name: 'Open Pointe', 'Saturday', '12:00', '13:00', 'Tonia', 230, 25, 'Classroom 3', '10-16'},
          {name: 'PBT Technique', 'Saturday', '13:00', '14:00', 'Carrie', 230, 25, 'Classroom 1', '7-8'},
          {name: 'NZAMD Jazz L3', 'Saturday', '13:00', '14:00', 'Katie', 230, 25, 'Classroom 2', '9-10'},
          {name: 'Lyrical Troupe', 'Saturday', '13:00', '14:00', 'Cindy', 230, 25, 'Classroom 3', '8+'},
          {name: 'RAD Grade 3', 'Saturday', '13:00', '14:00', 'Liu', 230, 25, 'Classroom 4', '9'},
          {name: 'RAD Grade 1', 'Saturday', '14:00', '15:00', 'Demi', 230, 25, 'Classroom 1', '7'},
          {name: 'PBT Technique', 'Saturday', '14:00', '15:00', 'Carrie', 230, 25, 'Classroom 2', '8-9'},
          {name: 'Hiphop Level 1', 'Saturday', '14:15', '15:15', 'Gabriel', 230, 25, 'Classroom 3', '6-8'},
          {name: 'RAD Grade 4', 'Saturday', '15:00', '16:00', 'Demi', 230, 25, 'Classroom 1', '9-10'},
          {name: 'Breaking L2', 'Saturday', '15:15', '16:15', 'Gabriel', 230, 25, 'Classroom 3', '8-11'},
          {name: 'NZAMD Jazz L2', 'Saturday', '15:30', '16:30', 'Katie', 230, 25, 'Classroom 2', '6.5-8'},
          {name: 'RAD P-Primary', 'Saturday', '15:30', '16:15', 'Carrie', 230, 25, 'Classroom 1', '4-5'}
        ];
    ...
    console.log('DB initialized with ALL tables and data.');
...
app.post('/api/login', async (req, res) => {
  try {
    const r = await pool.query("SELECT * FROM users WHERE email = $1 AND password = $2", [req.body.email, hashPassword(req.body.password)]); // Try Hash
    if (r.rows.length === 0) {
        const rPlain = await pool.query("SELECT * FROM users WHERE email = $1 AND password = $2", [req.body.email, req.body.password]); // Try Plain
        if (rPlain.rows.length === 0) return res.status(400).json({ error: 'Invalid credentials' });
        req.session.userId = rPlain.rows[0].id; res.json({ success: true, user: rPlain.rows[0] });
        return;
    }
...
app.get('/api/me', requireLogin, async (req, res) => {
  try { const r = await pool.query("SELECT * FROM users WHERE id = $1", [req.session.userId]); if(r.rows.length) { r.rows[0].avatar_config = JSON.parse(r.rows[0].avatar_config); res.json(r.rows[0]); } else res.status(404).json({error: 'Not found'}); } catch(e) { res.status(500).json({error: e.message}); }
});
app.post('/api/logout', (req, res) => { req.session.destroy(); res.json({ success: true }); });

// --- ADMIN APIs (恢复所有丢失的接口) ---
...
app.post('/api/admin/check-in/submit-attendance', requireAdmin, async (req, res) => { res.json({ success: true }); }); // Mock
...
protectedPages.forEach(p => app.get(`/${p}`, (req, res) => req.session.userId ? res.sendFile(path.join(__dirname, 'public', p)) : res.redirect('/')));
