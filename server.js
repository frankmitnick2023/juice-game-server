...
// ★★★ 修改点 1：管理员权限放开 (只要登录就是管理员) ★★★
function requireAdmin(req, res, next) {
  // 测试模式：只要有 userId (已登录)，就放行
...
// --- Security Page Interceptor (Protect Admin HTML) ---
...
// Static Files (Moved after interceptor)
...
// Experience Logic
async function accumulateExperience(userId, courseName) { const client = await pool.connect(); try { let category = 'Other'; if (courseName.includes('Ballet') || courseName.includes('RAD')) category = 'RAD Ballet'; else if (courseName.includes('Jazz') || courseName.includes('NZAMD')) category = 'NZAMD Jazz'; await client.query(`INSERT INTO course_progress (user_id, course_category, cumulative_hours) VALUES ($1, $2, $3) ON CONFLICT (user_id, course_category) DO UPDATE SET cumulative_hours = course_progress.cumulative_hours + $3`, [userId, category, 1.0]); } catch (e) { console.error(e); } finally { client.release(); } }
...
protectedPages.forEach(page => { app.get(`/${page}`, (req, res) => { if (req.session.userId) res.sendFile(path.join(__dirname, 'public', page)); else res.redirect('/'); }); });
